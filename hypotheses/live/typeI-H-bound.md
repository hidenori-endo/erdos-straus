# Type I H bound

- status: usable / treated as an unconditional result in the conversation

For Type I normalized parameters, define H = d(b-a)^2. The hard-prime congruence and residue conditions imply

~~~
H >= 22
~~~

and the top-strip estimate

~~~
k <= p - 2*sqrt(22*(p+1)/23) + 2
~~~

H=22 is recorded as a sharp boundary case. The remaining problem is to control the Type-I-only corridor, not to enumerate H blindly.
