...
- 関連: [character-annihilation-criterion](character-annihilation-criterion.md)、
  [../rejected/k35-pure-support-theorem.md](../rejected/k35-pure-support-theorem.md)
...
2. 全素因子が `ker χ` に入るなら、その積である全約数も `ker χ` に入る。
...
`k = 3,7,11,...,403` の 101 shift × hard prime 4,519 個 = 456,419 件で
「pure かつ hit」は 0 件。`k = 3,7,11,15` は `p < 10^8` の 179,468 件でも 0 件。
再現コードは [`code/audit.cpp`](../../code/README.md)。
...
逆向き（miss ⇒ pure）は一般には偽で、成立するのは
`k=3,7,15` の全 class と `k=11` の `h=169,289,529` の計 21 組のみ
