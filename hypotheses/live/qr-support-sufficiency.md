...
`χ(C_k) = jacobi(p,k) = ∏_{q|k} (p/q)^{v_q(k)}` で、hard prime は `mod 3,5,7` で平方なので、
`k` の奇数冪の素因子が全て `{3,5,7}` に入るなら `χ(C_k) = +1`、すなわち p-NR 素因子の
個数は偶数。該当する `k ≤ 403` は `3,7,15,35,27,63,75,147,243,315,363`。これらでは
「hit するなら p-NR 素因子が 2 個以上」に強まる
（[2026-09-03 seed 必要条件](../../updates/2026-09-03-seed-necessity-and-even-e.md) §1）。
